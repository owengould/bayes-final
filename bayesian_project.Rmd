...
d <- d %>% mutate(gender = ifelse(gender == 2, 0, 1), 
                  smoker_status = ifelse(smoker_status == 2, 0, 1)) %>% 
...
complex_regularized <- ulam(
    alist(
        blood_pressure ~ dnorm(mu, sigma),
        mu <- b0 + b1*gender + b2*smoker_status + b3*age + b4*age*gender + b5*age*smoker_status 
              + b6*std_bmi*smoker_status + b7*std_bmi + b8*sleep_hours,
        b0 ~ dnorm(0, 20),
        b1 ~ dnorm(0, 1), 
        b2 ~ dnorm(0, 1), 
        b3 ~ dnorm(0, 1), 
...
```{r}
precis(complex_regularized)
```

```{r}
precis(complex_informative)
```



```{r}
compare(basic_informative, basic_regularized, no_int, complex_regularized, complex_informative)
```
